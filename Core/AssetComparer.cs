...
{
    public sealed class AssetComparer<T> where T : Asset
    {
        ...
        private void UpdateAssetStates(IEnumerable<T> intersectingAssets)
        {
            foreach (T currentAsset in currentAssets)
            {
                ...
                {
                    currentAsset.SetState(AssetState.Missing);
                }
                else if (currentAsset.LastAssetRecord == null || currentAsset.LastAssetRecord.State != AssetState.Online)
                {
                    currentAsset.SetState(AssetState.Online);
                }
            }
        }
        ...
        private void UpdateAssetHolders(IEnumerable<T> intersectingAssets)
        {
            foreach (T newAsset in intersectingAssets)
            {
                ...
                if (currentAsset.CurrentAssetHolder == null || !currentAsset.CurrentAssetHolder.Equals(newAsset.CurrentAssetHolder))
                {
        ...
        private List<T> GetAddedAssets(IEnumerable<T> newAssets, IEnumerable<T> intersectingAssets)
        {
           return newAssets
               ...
               .Select(addedAsset =>
               {
                   addedAsset.SetState(AssetState.Online);
                   return addedAsset;
               })
               .ToList();
        }
